 ...
 */
async function getBatchInfo(client, batchId) {
 ...
 */
async function updateBatchEnrichmentStatus(client, batchId, status, details) {
    const params = [status];
    let paramIndex = 2;
 ...
 */
async function checkFileEnrichmentCompletion(client, fileId) {
    try {
 ...
 */
async function invokeExclusionsProcessor(fileId, opportunityId) {
    try {
 ...
 */
async function fetchClaimRecords(client, fileId, startRow, endRow) {
    const result = await client.query(`
    SELECT 
 ...
 */
async function processClaims(client, claims, processors) {
    let enriched = 0;
    let failed = 0;
 ...
 */
async function updateClaimWithEnrichment(client, recordId, dynamicFields) {
    await client.query(`
 ...
 */
async function handleProcessingError(client, batchId, error) {
    const errorDetails = {
